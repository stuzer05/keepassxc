...
bool YkChallengeResponseKey::challenge(const QByteArray& challenge)
{
    ...
    QByteArray response;
    auto result =
        AsyncTask::runAndWaitForFuture([&] { return YubiKey::instance()->challenge(m_keySlot, challenge, response); });

    if (result == YubiKey::SUCCESS) {
        m_key.resize(response.size());
        Botan::copy_mem(m_key.data(), response.constData(), response.size());
        Botan::secure_scrub_memory(response.data(), response.capacity());
    } else {
